...
shop_slide = (direction) => {
  if (
    (direction === "next" && shop_translation_index === 3) ||
    (direction === "prev" && shop_translation_index === 1)
  ) {
    return;
  } else {
    direction === "next" ? shop_translation_index++ : shop_translation_index--;
    direction === "next"
      ? (translate_shop -= prendas_shop_translate)
      : (translate_shop += prendas_shop_translate);

    prendas_shop.forEach(
      (prendas_shop) =>
        (prendas_shop.style.transform = `translateX(${translate_shop}%)`)
    );
  }
};
...
cantera_slide = (direction) => {
  if (
    ...
    direction === "next"
      ...
      : cantera_translation_index--;
    direction === "next"
      ? (translate_cantera -= equipos_cantera_translate / 2)
      : (translate_cantera += equipos_cantera_translate / 2);
